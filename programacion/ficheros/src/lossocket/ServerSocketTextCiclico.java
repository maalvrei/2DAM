...
public class ServerSocketTextCiclico {
	...
	public ServerSocketTextCiclico(int puerto) throws IOException {
	...
	public static void main(String[] args) {
		ServerSocketTextCiclico server;
		try {
			label: while (true) {
				server = new ServerSocketTextCiclico(8081);
				server.start();
				server.abrirCanalesDeTexto();
				while (true) {
					String datoLeido = server.br.readLine();
					System.out.printf("(Servidor) Leido: %s%n", datoLeido);
					if (datoLeido.equals("fin")) break;
					if (datoLeido.equals("xxx")) break label;
